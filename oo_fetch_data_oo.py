...
def generate_and_insert_pci_report(conn, cur):
    """Generate PCI report from zoho_accounts_table and insert into pci_report_table."""

    print("\n📢 Generating PCI Report from zoho_accounts_table...\n")

    cur.execute("SELECT merchant_number, account_name, sales_id, outside_agents, date_approved, pci_fee, pci_amnt, split FROM zoho_accounts_table WHERE merchant_number IS NOT NULL AND pci_fee IS NOT NULL AND pci_amnt IS NOT NULL AND split IS NOT NULL")
    ...
    for row in rows:
        ...
        try:
            pci_fee = float(pci_fee)
            pci_amnt = float(pci_amnt)
            ...
            effective_month = approval_month  # Use same for now, or adjust if logic changes
